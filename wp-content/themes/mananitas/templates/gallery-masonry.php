		...
		<ul class="image-main-box clearfix">
			<?php 
				foreach ($gallery_items_id as $gallery_item_id) 
				{
					...
					echo '
						<li class="item '.(isset($image->post_title) && $image->post_title !=='' ? esc_attr($image->post_title) : 'col-xs-6 col-md-3').' '.(isset($image->post_content) && $image->post_content !=='' ? esc_attr(str_replace(' ','-',trim(strtolower($image->post_content)))) : '').'">
							<figure>
